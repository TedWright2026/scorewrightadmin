...
function StablefordLiveScores({ selectedComp, teams, players, scores, courses, compCourses, format }) {
  ...
  const teamRows = teams.map(team => {
    ...
    for (let hIdx = 0; hIdx < 18; hIdx++) {
      ...
      const ptsGross = tpEnriched.map(p => {
        const sc = scores.find(s => s.team_id === team.id && s.player_slot === p.slot && s.hole_index === hIdx);
        if (!sc || sc.gross_score == null) return null;
        const h = p.course?.holes?.[hIdx];
        if (!h) return null;
        return stbPts(sc.gross_score, h.par, 0);  // scratch
      }).filter(x => x !== null);
      ...
      if (ptsGross.length > 0) {
        const topN = [...ptsGross].sort((a,b) => b-a).slice(0, bestN);
        grossTotal += topN.reduce((s,x) => s+x, 0);
      }
      if (ptsNett.length > 0 || ptsGross.length > 0) holesScored++;
    }
    return { ...team, total, grossTotal, holesScored, playerCount: tp.length };
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <Card>
        ...
        <table>
          ...
          <tbody>
            {teamRows.length===0 ? <tr><td colSpan={6} style={{textAlign:"center",color:T.textDk,padding:30}}>No teams yet</td></tr> :
              teamRows.map((t,i) => (
                <tr key={t.id} style={{background:i===0?`${T.green}11`:"transparent"}}>
                  ...
                  <td style={{textAlign:"center",fontWeight:700,fontSize:15,color:T.textMd}}>{t.holesScored>0?(t.grossTotal ?? "–"):"–"}</td>
